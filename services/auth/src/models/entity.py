...
class DateTimeBaseModel(BaseModel):
    ...
    modified_at = Column(TIMESTAMP, nullable=False, onupdate=func.now(), server_default=func.now())
...
class RefreshTokens(BaseModel):
    ...
    user_uuid = Column(UUID(as_uuid=True), ForeignKey(f"{AUTH_SCHEMA}.users.uuid"))
    created_at = Column(TIMESTAMP, nullable=False, server_default=func.now())
    expiration_date = Column(TIMESTAMP, nullable=False, default=lambda: datetime.now() + timedelta(days=7))
