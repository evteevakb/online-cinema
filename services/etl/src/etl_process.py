...
class ETLSettings(BaseSettings):
    ...
    index_schema_filepath: str = "es_schema.json"
...
while True:
    for table_name in ["person", "genre", "film_work"]:
        logger.info("Start ETL process for %s table", table_name)
        state = state_storage.retrieve_state(table_name)
        if state[table_name] is None:
        ...
        with closing(
            ...
            while True:
                ...
                if len(batch) == 0:
                    logger.info("No new data for table %s, skipping", table_name)
                    break
                elastic_batch = [transform_postgres_to_elastic(row) for row in batch]
            ...
            logger.info("ETL process for %s table finished", table_name)
    logger.info(
